 ...
 *   - This wrapper is located at the repository root (orkee-oss/)
 *   - This wrapper uses cwd to spawn processes from packages/dashboard/ where package.json is located
...
// Get the directory containing this script (repository root)
const __dirname = dirname(fileURLToPath(import.meta.url));
// Resolve to packages/dashboard where bun run dev should execute
const dashboardDir = join(__dirname, 'packages', 'dashboard');
...
viteProcess = spawn(command, ['run', 'dev'], {
  ...
  cwd: dashboardDir, // Run from packages/dashboard where package.json is located
});
