// ABOUTME: Modal provider stub for serverless data/ML workloads
...
impl ModalProvider {
    ...
    pub fn new(
        ...
        Ok(Self {
            token_id,
            token_secret,
            api_endpoint: api_endpoint.unwrap_or_else(|| "https://api.modal.com".to_string()),
        })
    }

    fn not_supported(&self) -> ProviderError {
        ProviderError::NotSupported(
            "Modal provider is not yet implemented. Only Local Docker is currently supported. \
             To implement: add modal REST API client and implement the Provider trait methods."
                .to_string(),
        )
    }
}
...
impl Provider for ModalProvider {
    async fn is_available(&self) -> Result<bool> {
        warn!("Modal provider is not yet implemented");
        Ok(false)
    }
