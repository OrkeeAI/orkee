...
impl<'a> Widget for MentionPopupWidget<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for (row_idx, mention_match) in matches.iter().enumerate().take(display_count as usize) {
            ...
            if x < inner.x + inner.width {
                buf[(x, y)].set_char('-').set_style(bg_style.fg(Color::Gray));
                x += 1;
            }
            if x < inner.x + inner.width {
                buf[(x, y)].set_char(' ').set_style(bg_style);
            ...
            let truncated_path = if path.len() > available_width as usize {
                format!("...{}", &path[path.len().saturating_sub(available_width.saturating_sub(3) as usize)..])
            } else {
                path.clone()
            };
            ...
            if let Some(description) = &mention_match.item.description {
                let available_width = (inner.x + inner.width).saturating_sub(x);
                if available_width > 4 { // Need at least space for " (x)"
                    ...
                    let truncated_desc = if description.len() > available_for_desc as usize {
                        format!("{}...", &description[..available_for_desc.saturating_sub(3) as usize])
                    } else {
                        description.clone()
                    };
                    
                    for ch in truncated_desc.chars() {
                        if x >= inner.x + inner.width - 1 { // Leave space for closing paren
                            break;
                        }
                        buf[(x, y)]
                            .set_char(ch)
...
mod tests {
    use super::*;
    use crate::mention_popup::{MentionPopup, MentionItem, MentionTarget};
