...
impl MentionPopup {
    ...
    pub fn from_projects(projects: &[Project], mention_start_position: usize) -> Self {
        let items = projects.iter().map(MentionItem::from_project).collect();
...
mod tests {
    ...
    fn create_test_project(id: &str, name: &str, path: &str) -> Project {
        Project {
            ...
            status: orkee_projects::ProjectStatus::Planning,
    ...
    fn test_selected_item() {
        let projects = vec![create_test_project("1", "project-one", "/path/one")];
