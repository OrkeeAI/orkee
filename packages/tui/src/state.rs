use crate::chat::{ChatMessage, MessageHistory};
...
use crate::ui::widgets::{FormField, FormStep, FormWidget};
use orkee_projects::{
    create_project, delete_project, update_project, Priority, Project, ProjectCreateInput,
    ProjectStatus, ProjectUpdateInput,
};
...
impl AppState {
    pub fn new(refresh_interval: u64) -> Self {
        ...
        state
            .message_history
            .add_system_message("Welcome to Orkee TUI! Type a message to get started.");
        state
    }
    ...
    pub fn start_project_creation(&mut self) {
        ...
        step1.add_field(FormField::multiline_text(
        ...
        step2.add_field(FormField::selection(
            "status".to_string(),
            "Status".to_string(),
        ...
        step2.add_field(FormField::tags(
            "tags".to_string(),
            "Tags".to_string(),
            false,
        ));
    ...
    fn create_edit_form(&self, project: &Project) -> FormWidget {
        ...
        let mut priority_field = FormField::selection(
            "priority".to_string(),
            "Priority".to_string(),
            priority_options,
            false,
        );
        if let FieldValue::Selection {
            ref mut selected, ..
        } = &mut priority_field.field_value
        {
    ...
    pub fn form_current_field_is_multiline(&self) -> bool {
        if let Some(ref form_state) = self.form_state {
            if let Some(field) = form_state.form.current_field() {
                ...
                let is_multiline_type = matches!(
                    field.field_type,
                    crate::ui::widgets::FieldType::MultilineText
                );
...
impl AppState {
    ...
    pub fn handle_dialog_key(&mut self, key: crossterm::event::KeyCode) -> Option<DialogResult> {
        if let Some(dialog) = &mut self.confirmation_dialog {
            ...
            match result {
                DialogResult::Confirmed | DialogResult::Cancelled => Some(result),
        ...
        match delete_project(&project_id).await {
            Ok(true) => {
                ...
                match orkee_projects::get_all_projects().await {
                    ...
                    Err(e) => Err(format!("Project deleted but failed to refresh list: {}", e)),
                }
            }
            Ok(false) => {
                // Project was not found (already deleted?)
                Err(format!(
                    "Project \"{}\" was not found (may have been already deleted)",
                    project_name
                ))
            }
            Err(e) => {
    ...
    pub fn update_search(&mut self) {
        if let Some(ref mut search_popup) = self.search_popup {
            ...
            if !results.is_empty()
                || search_popup.has_active_filters()
                || !search_popup.search_query().is_empty()
            {
                self.filtered_project_indices =
                    Some(results.iter().map(|r| r.project_index).collect());
