...
pub fn format_projects_table(projects: &[Project], current_project: Option<&Project>) -> String {
    ...
    let max_name_length = std::cmp::max(20, projects.iter().map(|p| p.name.len()).max().unwrap_or(0));
    let max_path_length = std::cmp::max(30, projects.iter().map(|p| p.project_root.len()).max().unwrap_or(0));
    let max_status_length = std::cmp::max(8, projects.iter().map(|p| format!("{:?}", p.status).len()).max().unwrap_or(0));
    ...
    let rows: Vec<String> = projects
        ...
        .map(|project| {
            ...
            let name = format!("{:<width$}", truncate(&project.name, max_name_length), width = max_name_length);
            let path = format!("{:<width$}", truncate(&project.project_root, max_path_length), width = max_path_length);
            let status = format!("{:<width$}", format!("{:?}", project.status).to_lowercase(), width = max_status_length);
...
pub fn format_project_details(project: &Project) -> String {
    let mut lines = vec![
        ...
        format!("Created: {}", project.created_at.format("%m/%d/%Y, %l:%M:%S %p")),
        format!("Updated: {}", project.updated_at.format("%m/%d/%Y, %l:%M:%S %p")),
    ];
    ...
    if let Some(ref priority) = Some(&project.priority) {
        lines.push(format!("Priority: {}", format!("{:?}", priority).to_lowercase()));
    }
    ...
    if let Some(ref task_source) = project.task_source {
        lines.push(format!("Task Source: {}", format!("{:?}", task_source).to_lowercase()));
    }
    ...
    if let Some(ref mcp_servers) = project.mcp_servers {
        ...
        if total_count > 0 {
            lines.push(format!("MCP Servers: {}/{} enabled", enabled_count, total_count));
        }
    }
...
mod tests {
    use super::*;
    use crate::types::{ProjectStatus, Priority};
    use chrono::Utc;
