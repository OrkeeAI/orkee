use crate::{
    providers::{CloudProvider, CloudResult, CloudError, SnapshotId, SnapshotMetadata, UploadOptions, 
               DownloadOptions, ListOptions, AuthToken},
    types::{SyncResult, SyncOperation, ValidationResult}
};
...
/// Minimal interface for local storage operations needed by cloud sync
#[async_trait::async_trait]
pub trait LocalStorage: Send + Sync {
    /// Export current state as a snapshot
    async fn export_snapshot(&self) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>>;
    
    /// Import snapshot data to restore state
    async fn import_snapshot(&self, data: &[u8]) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    
    /// List projects for metadata collection
    async fn list_projects(&self) -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>>;
}
...
impl SyncEngine {
    ...
    pub async fn backup(&self) -> CloudResult<SyncResult> {
        ...
        match self.perform_backup_internal().await {
            ...
            Err(error) => {
                ...
                let _failed_result = SyncResult::failed(started_at, error.to_string());
    ...
    pub async fn list_snapshots(&self) -> CloudResult<Vec<crate::providers::SnapshotInfo>> {
    ...
    pub async fn get_cloud_usage(&self) -> CloudResult<crate::providers::StorageUsage> {
        let auth_token = self.get_valid_auth_token().await?;
    ...
    async fn perform_backup_internal(&self) -> CloudResult<SyncResult> {
        ...
        let snapshot_id = crate::providers::generate_snapshot_id();
        let checksum = crate::providers::calculate_checksum(&snapshot_data);
    ...
    async fn perform_restore_internal(&self, snapshot_id: &SnapshotId) -> CloudResult<SyncResult> {
        ...
        let _import_result = self.local_storage
        ...
        // Note: Import completed successfully - no detailed conflict information available from LocalStorage trait
        
        Ok(SyncResult::completed(
            started_at,
            snapshot_id.clone(),
            snapshot_data.len() as u64,
            1, // Default to 1 project imported since we can't get detailed stats from LocalStorage trait
        ))
    }
    ...
    pub async fn validate_snapshot(&self, snapshot_id: &SnapshotId) -> CloudResult<ValidationResult> {
        let auth_token = self.get_valid_auth_token().await?;
        ...
        // Validate checksum
        let actual_checksum = crate::providers::calculate_checksum(&snapshot_data);
...
impl SyncEngineFactory {
    ...
    pub async fn create_s3_sync_engine(
        ...
        let s3_provider = crate::providers::s3::S3Provider::new(bucket, region);
    ...
    pub async fn create_r2_sync_engine(
        ...
        let r2_provider = crate::providers::s3::S3Provider::new_r2(bucket, account_id);
...
mod tests {
    use super::*;
    // Mock storage for testing since we don't depend on projects package
    ...
    mock! {
        ...
        impl CloudProvider for CloudProvider {
            ...
            async fn list_snapshots(&self, token: &AuthToken, options: ListOptions) -> CloudResult<Vec<crate::providers::SnapshotInfo>>;
            async fn get_snapshot_info(&self, token: &AuthToken, id: &SnapshotId) -> CloudResult<crate::providers::SnapshotInfo>;
            async fn delete_snapshot(&self, token: &AuthToken, id: &SnapshotId) -> CloudResult<()>;
            async fn get_storage_usage(&self, token: &AuthToken) -> CloudResult<crate::providers::StorageUsage>;
