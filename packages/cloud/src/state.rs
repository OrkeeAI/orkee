// Define storage error types for cloud state management
use thiserror::Error;
...
pub type CloudStorageResult<T> = Result<T, CloudStorageError>;
...
impl CloudSyncStateManager {
    ...
    pub async fn upsert_provider_state(
        ...
        provider_name: &str,
        provider_type: &str,
    ) -> CloudStorageResult<CloudSyncState> {
        let now = Utc::now();
        ...
        self.get_provider_state(provider_name).await?
            .ok_or(CloudStorageError::InvalidState("Provider not found".to_string()))
    }

    /// Get cloud sync state for a provider
    pub async fn get_provider_state(&self, provider_name: &str) -> CloudStorageResult<Option<CloudSyncState>> {
        ...
        match record {
            Some(row) => Ok(Some(CloudSyncState {
                ...
                last_sync_at: row.last_sync_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
                last_successful_sync_at: row.last_successful_sync_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
                ...
                last_error_at: row.last_error_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
    ...
    pub async fn update_sync_state(
        ...
        let now = Utc::now();
        let last_sync_rfc = last_sync_at.map(|dt| dt.to_rfc3339());
        let last_successful_sync_rfc = last_successful_sync_at.map(|dt| dt.to_rfc3339());
        let now_rfc = now.to_rfc3339();
    ...
    pub async fn list_snapshots(
        ...
        provider_name: &str,
        limit: Option<u32>,
    ) -> CloudStorageResult<Vec<CloudSnapshot>> {
        ...
        for row in records {
            snapshots.push(CloudSnapshot {
                ...
                etag: row.etag,
                last_accessed_at: row.last_accessed_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
                uploaded_at: row.uploaded_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
                download_count: row.download_count.unwrap_or(0),
                last_downloaded_at: row.last_downloaded_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
                locally_deleted: row.locally_deleted.unwrap_or(false),
                deletion_scheduled_at: row.deletion_scheduled_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
    ...
    pub async fn get_pending_conflicts(&self, provider_name: &str) -> CloudStorageResult<Vec<SyncConflict>> {
        let records = sqlx::query!(
            r#"
        ...
        for row in records {
            conflicts.push(SyncConflict {
                ...
                resolved_at: row.resolved_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
    ...
    pub async fn get_sync_health_summary(&self) -> CloudStorageResult<Vec<SyncHealthSummary>> {
        let records = sqlx::query!(
            r#"
            SELECT 
                css.provider_name,
        ...
        for row in records {
            summaries.push(SyncHealthSummary {
                ...
                latest_snapshot_at: row.latest_snapshot_at.as_ref().and_then(|s| DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc))),
            });
        }
