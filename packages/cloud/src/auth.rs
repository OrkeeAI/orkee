...
impl AuthManager {
    ...
    pub async fn get_valid_token(&self) -> CloudResult<String> {
        match &self.token_info {
            ...
            _ => Err(CloudError::auth(
                "No valid token available. Please run 'orkee cloud login'",
            )),
        }
    }
    ...
    pub fn user_info(&self) -> Option<(&str, &str, &str)> {
        self.token_info.as_ref().filter(|t| t.is_valid()).map(|t| {
            (
                t.user_id.as_str(),
                t.user_email.as_str(),
                t.user_name.as_str(),
    ...
    async fn refresh_token_internal(&self, _current_token: String) -> CloudResult<String> {
        ...
        Err(CloudError::auth(
            "Token expired. Please run 'orkee cloud login' again",
        ))
    }
