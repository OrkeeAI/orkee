...
async fn test_health_check_returns_ok() {
    ...
    assert_eq!(value.get("service").and_then(|v| v.as_str()), Some("orkee-cli"));
...
async fn test_health_endpoints_consistency() {
    ...
    assert_eq!(
        health_value.get("service"),
        status_value.get("service")
    );
    assert_eq!(
        health_value.get("version"),
        status_value.get("version")
    );
}
