...
fn build_redirect_url(
    ...
    let host = if config.preserve_host {
        ...
        if let Some(host_header) = headers.get("host") {
            host_header.to_str().map_err(|_| StatusCode::BAD_REQUEST)?
        } else {
            "localhost"
        }
    } else {
...
mod tests {
    ...
    fn test_build_redirect_url_complex_query() {
        let uri = "http://localhost:4000/api/test?param1=value1&param2=value%202&param3="
            .parse::<Uri>()
            .unwrap();
        let config = HttpsRedirectConfig {
            enabled: true,
        ...
        assert_eq!(
            redirect_url,
            "https://localhost:4443/api/test?param1=value1&param2=value%202&param3="
        );
    }
