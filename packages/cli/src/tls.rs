...
impl TlsManager {
    ...
    fn should_generate_certificate(&self) -> Result<bool, TlsError> {
        ...
        match self.check_certificate_validity() {
            ...
            Err(e) => {
                warn!(
                    "Failed to check certificate validity: {}, will regenerate",
                    e
                );
                Ok(true)
            }
    ...
    async fn generate_self_signed_certificate(&self) -> Result<(), TlsError> {
        ...
        params
            .distinguished_name
            .push(rcgen::DnType::CommonName, "Orkee Development Certificate");
        params
            .distinguished_name
            .push(rcgen::DnType::OrganizationName, "Orkee");
        params
            .distinguished_name
...
impl std::fmt::Display for TlsConfigInfo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "TLS enabled: {}, auto-generate: {}, cert exists: {}, key exists: {}",
