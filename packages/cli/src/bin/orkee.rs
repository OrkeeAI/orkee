...
async fn handle_command(command: Commands) -> Result<(), Box<dyn std::error::Error>> {
    match command {
        Commands::Dashboard { port, cors_origin, restart } => {
            if restart {
        ...
        Commands::Preview(preview_cmd) => {
            handle_preview_command(preview_cmd).await
        }
    }
}

async fn handle_preview_command(command: PreviewCommands) -> Result<(), Box<dyn std::error::Error>> {
    match command {
        PreviewCommands::StopAll => {
            stop_all_preview_servers().await
        }
    }
}
...
async fn stop_all_preview_servers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client.get("http://localhost:4001/api/preview/servers")
        .send()
        .await?;
    
    if !response.status().is_success() {
        return Err("Failed to connect to Orkee server. Make sure the dashboard is running.".into());
    }
    
    let server_list: serde_json::Value = response.json().await?;
    let project_ids = server_list["data"].as_array()
    ...
    println!("{} Found {} running server(s)", "📋".cyan(), project_ids.len());
    ...
    for project_id in project_ids {
        ...
        let stop_response = client.post(&format!("http://localhost:4001/api/preview/servers/{}/stop", id))
            .send()
            .await;
    ...
    if failed == 0 {
        println!("{} Successfully stopped all {} preview servers!", "🎉".green(), stopped);
    } else {
        println!("{} Stopped {} servers, {} failed", "⚠️".yellow(), stopped, failed);
    }
...
async fn start_full_dashboard(port: u16, cors_origin: String) -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "🚀 Starting Orkee Dashboard (Backend + Frontend)...".green().bold());
    ...
    match frontend_result {
        ...
        Err(e) => {
            ...
            eprintln!("{} Make sure you're in the packages/cli directory and pnpm is installed", "Tip:".yellow());
        }
    }
...
async fn restart_dashboard(port: u16, cors_origin: String) -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "🔄 Restarting all dashboard services...".yellow().bold());
