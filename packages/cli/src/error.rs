...
impl AppError {
    ...
    fn to_status_and_code(&self) -> (StatusCode, &'static str) {
        match self {
            ...
            AppError::Storage(manager_error) => {
                match manager_error {
                    orkee_projects::manager::ManagerError::NotFound(_) => (StatusCode::NOT_FOUND, "RESOURCE_NOT_FOUND"),
                    orkee_projects::manager::ManagerError::DuplicateName(_) => (StatusCode::CONFLICT, "DUPLICATE_NAME"),
                    orkee_projects::manager::ManagerError::DuplicatePath(_) => (StatusCode::CONFLICT, "DUPLICATE_PATH"),
                    orkee_projects::manager::ManagerError::Validation(_) => (StatusCode::BAD_REQUEST, "VALIDATION_ERROR"),
                    orkee_projects::manager::ManagerError::Storage(_) => (StatusCode::INTERNAL_SERVER_ERROR, "STORAGE_ERROR"),
                }
            },
            AppError::PathValidation(validation_error) => {
                match validation_error {
                    crate::api::path_validator::ValidationError::PathTraversal => (StatusCode::FORBIDDEN, "PATH_TRAVERSAL"),
                    crate::api::path_validator::ValidationError::RootAccess => (StatusCode::FORBIDDEN, "ROOT_ACCESS"),
                    crate::api::path_validator::ValidationError::BlockedPath(_) => (StatusCode::FORBIDDEN, "BLOCKED_PATH"),
                    crate::api::path_validator::ValidationError::SensitiveDirectory(_) => (StatusCode::FORBIDDEN, "SENSITIVE_DIRECTORY"),
                    crate::api::path_validator::ValidationError::NotInAllowedPaths => (StatusCode::FORBIDDEN, "NOT_IN_ALLOWED_PATHS"),
                    crate::api::path_validator::ValidationError::SymlinkError => (StatusCode::FORBIDDEN, "SYMLINK_ERROR"),
                    crate::api::path_validator::ValidationError::InvalidPath => (StatusCode::BAD_REQUEST, "INVALID_PATH"),
                    crate::api::path_validator::ValidationError::PathDoesNotExist => (StatusCode::NOT_FOUND, "PATH_NOT_FOUND"),
                    crate::api::path_validator::ValidationError::ExpansionError => (StatusCode::BAD_REQUEST, "PATH_EXPANSION_ERROR"),
                }
            },
        }
    }
    ...
    fn to_user_message(&self) -> String {
        match self {
            ...
            AppError::Storage(manager_error) => {
                match manager_error {
                    orkee_projects::manager::ManagerError::NotFound(resource) => format!("{} not found", resource),
                    orkee_projects::manager::ManagerError::DuplicateName(name) => format!("A project with the name '{}' already exists", name),
                    orkee_projects::manager::ManagerError::DuplicatePath(path) => format!("A project already exists at path '{}'", path),
                    orkee_projects::manager::ManagerError::Validation(errors) => {
                        format!("Validation failed: {} error(s)", errors.len())
                    },
                    orkee_projects::manager::ManagerError::Storage(_) => "Data storage error".to_string(),
                }
            },
            AppError::PathValidation(validation_error) => {
                match validation_error {
                    crate::api::path_validator::ValidationError::PathTraversal => "Path traversal detected and blocked".to_string(),
                    crate::api::path_validator::ValidationError::RootAccess => "Root directory access denied".to_string(),
                    crate::api::path_validator::ValidationError::BlockedPath(_) => "Access denied to blocked path".to_string(),
                    crate::api::path_validator::ValidationError::SensitiveDirectory(_) => "Access to sensitive directory blocked".to_string(),
                    crate::api::path_validator::ValidationError::NotInAllowedPaths => "Path is outside allowed directories".to_string(),
                    crate::api::path_validator::ValidationError::SymlinkError => "Symlink validation failed".to_string(),
                    crate::api::path_validator::ValidationError::InvalidPath => "Invalid path provided".to_string(),
                    crate::api::path_validator::ValidationError::PathDoesNotExist => "Path does not exist".to_string(),
                    crate::api::path_validator::ValidationError::ExpansionError => "Path expansion failed".to_string(),
                }
            },
        }
    }
    ...
    fn into_response(self) -> Response {
        ...
        match &self {
            ...
            AppError::Storage(manager_err) => {
                if matches!(manager_err, orkee_projects::manager::ManagerError::Storage(_)) {
                    error!(
                        request_id = %request_id,
...
mod tests {
    ...
    fn test_user_message_sanitization() {
        let internal_error = AppError::internal(anyhow::anyhow!("Database connection failed with password xyz"));
