 ...
 * Adapted from vibekit for orkee-oss.
 */
...
/**
 * Get the default editor settings
 */
...
/**
 * Get current editor settings from localStorage
 */
export const getEditorSettings = (): EditorSettings => {
  try {
    const stored = localStorage.getItem('orkee-editor-settings');
    if (stored) {
      return { ...getDefaultEditorSettings(), ...JSON.parse(stored) };
    }
  } catch (error) {
    console.warn('Failed to parse editor settings from localStorage:', error);
  }
...
export function getAvailableEditors(): EditorConfig[] {
  // Since this runs in the browser, we'll show all editors
  // Platform filtering will be done on the backend
  return SUPPORTED_EDITORS;
}

/**
 * Find editor config by ID
 */
export function findEditorById(editorId: string): EditorConfig | undefined {
  return SUPPORTED_EDITORS.find(editor => editor.id === editorId);
}