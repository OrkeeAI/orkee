...
async fn import_claude_from_setup() -> Result<(), String> {
    println!("🔐 Running 'claude setup-token' to generate authentication token...");
...
async fn import_claude_from_file(file_path: &str) -> Result<(), String> {
    println!("📄 Importing Claude token from file: {}", file_path);
...
📄 Importing Claude token from file: /tmp/token.txt
...
📄 Importing Claude token from file: /tmp/bad.txt
❌ Error: Invalid token format. Claude OAuth tokens should start with 'sk-ant-oat01-'.
...
- ⚠️ Machine-based encryption provides transport security only
- ⚠️ Password-based encryption recommended for production
