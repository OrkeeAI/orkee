...
#### Tasks
- [ ] Research Claude OAuth endpoints and requirements
- [ ] Implement Claude-specific OAuth flow
- [ ] Handle Claude Pro/Max subscription detection
- [ ] Test with real Claude account
- [ ] Add error handling for Claude-specific errors
...
- [ ] Implement xAI/Grok OAuth flow
- [ ] Add provider-specific error handling
- [ ] Test each provider with real accounts

### 2.3 CLI Commands

#### Tasks
- [ ] Create `packages/cli/src/bin/cli/auth.rs` module
- [ ] Implement `orkee login <provider>` command
- [ ] Implement `orkee logout <provider>` command
- [ ] Implement `orkee auth status` command
- [ ] Implement `orkee auth refresh <provider>` command
- [ ] Add `--force` flag for re-authentication
- [ ] Add provider selection prompt if no provider specified
- [ ] Add success/error messages with proper formatting
